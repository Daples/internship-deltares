...
srun --time 00:20:00 --nodes 1 --exclusive -n $nPart $singularitydir/execute_singularity_snellius.sh -p 0 run_dflowfm.sh $mdufile > dimr.log
printf "TIMINGS $(basename $PWD): $(date) WAITING FOR SUBMIT COMMAND TO FINISH\n"
wait

#If the run has somehow become stuck and has not finished within 6 minutes, resubmit the run until is does finish
while ! test -f "output/gtsm_fine_0000_his.nc"
do
    echo "TIMINGS $(basename $PWD): WARNING: DFLOW-FM output not available, might have crashed, resubmitting at $(date)"
    srun --time 00:20:00 --nodes 1 --exclusive -n $nPart $singularitydir/execute_singularity_snellius.sh -p 0 run_dflowfm.sh $mdufile > dimr.log
    wait
    echo "TIMINGS $(basename $PWD): WARNING: (2) srun command finished at                                       $(date)"
    sleep 10s
    ((c++)) && ((c==10)) && break
done
